...
export default function Header() {
  ...
  return (
    <div className="w-full mb-[96px]">
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="lg:hidden flex items-center gap-2">
              {!isOpen ? (
                <Link
                  href="/kontakt"
                  className="flex gap-2 border py-1  bg-stone-700 text-white rounded-full px-3 font-normal items-center"
              ...
              >
                ...
                <div className={`w-6 h-6 flex flex-col justify-around items-center${isOpen ? ' text-red-600' : ''}`}>
                  <span
                    className={`block w-full h-0.5 bg-current transform transition duration-300 ease-in-out ${isOpen ? 'rotate-45 translate-y-[7.8px]' : ''
                      }`}
                  ></span>
                  <span
      ...
      <div
        className={`lg:hidden fixed inset-0 bg-white z-40 transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'
          }`}
        id="mobile-menu"
      >
        <div className="h-full flex flex-col items-center justify-center pt-[96px]">
            ...
            <nav className="flex flex-col items-center space-y-8">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="text-sm uppercase text-gray-800 hover:text-black text-center"
                  onClick={() => setIsOpen(false)}
                >
                  {link.label}
                  <div className='mt-2 bg-accent-dark w-[180px] h-[1px]'></div>
                </Link>
              ))}
